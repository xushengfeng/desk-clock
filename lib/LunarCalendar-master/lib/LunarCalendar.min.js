...
! function () {
    ...
    function c(a, c) {
        for (var d = b(a), e = c > 0 ? c : d.yearDays - Math.abs(c), f = d.monthDays, g = 0, h = 0, i = 0; i < f.length; i++)
            if (g += f[i], g > e) {
                h = i, g -= f[i];
                break
            } return [a, h, e - g + 1]
    ...
    function r(c, e, f) {
        ...
        var r = z.get("term2") ? z.get("term2") : z.set("term2", g(n, 2)),
            ...
            x = a(w[0]),
            y = "";
        y = x > 0 && x == w[1] ? "闰" + D.monthCn[w[1] - 1] + "月" : x > 0 && w[1] > x ? D.monthCn[w[1] - 1] + "月" : D.monthCn[w[1]] + "月";
        var C = "",
            H = b(w[0]).monthDays;
